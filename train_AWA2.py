...
parser.add_argument('--nSample', type=int, default=60000, help='number features to generate per class')
...
def train():
    ...
    for it in range(start_step, opt.niter+1):
        ...
        if it % opt.evl_interval == 0 and it > 10000:
            flow.eval()
            sm.eval()
            ...
            np.save(opt.dataset+str(it)+'.npy', gen_feat.cpu().numpy())
