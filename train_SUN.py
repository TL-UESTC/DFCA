...
parser.add_argument('--nSample', type=int, default=120, help='number features to generate per class')
...
parser.add_argument('--manualSeed', type=int, default=17615, help='manual seed')
...
parser.add_argument('--cons',    type=float, default=8, help='weight of the prototype loss')
...
def train():
    ...
    out_dir = 'out/{}/s_d-{}_mask-{}_cl-{}_c-{}_ns-{}_wd-{}_lr-{}_nS-{}_bs-{}_ps-{}'.format(opt.dataset, opt.input_dim, opt.dropout, opt.num_coupling_layers, opt.prototype,
    ...
    for it in range(start_step, opt.niter+1):
        ...
        if it % opt.evl_interval == 0 and it > 3000:
            flow.eval()
            sm.eval()
...
class AttDec(nn.Module):
    def __init__(self, opt):
        ...
        self.embedSz = 0
        self.fc1 = nn.Linear(opt.X_dim, opt.ngh)
        self.fc3 = nn.Linear(opt.ngh, opt.input_dim)
